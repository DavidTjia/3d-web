...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const wardekaTrigger = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Sinkronkan fade teks dengan objek 3D — sama-sama pakai progress yang sama,
        // jadi keduanya "masuk" dan "keluar" barengan, bukan gantian.
        if (wardekaTextRef.current) {
          const p = self.progress;
          let opacity = 1;
          if (p < 0.12) opacity = p / 0.12;
          else if (p > 0.85) opacity = Math.max(0, 1 - (p - 0.85) / 0.15);

          wardekaTextRef.current.style.opacity = opacity.toString();
          wardekaTextRef.current.style.transform = `translate3d(0, ${(1 - opacity) * 20}px, 0)`;
        }
      },
    });
  ...
  return (
    <>
      ...
      {/* 2D Spider Creature Overlay — fade-out otomatis begitu keluar dari hero */}
            ...
            >
              <div className="max-w-2xl">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-2 mb-6 max-w-xl text-xs text-gray-400">
                  ...
                  <p>
                    ⚙ Dashboard & analytic custom (performa, item, penjualan)
                  </p>
                  <p>
